...
export class RoleDetectionService {
  ...
  async analyzeRoleCompatibility(cv: ParsedCV, roleId: string): Promise<RoleProfileAnalysis> {
    try {
      ...
      const profile = await this.roleProfileService.getProfileById(roleId);
      if (!profile) {
        throw new Error(`Role profile not found: ${roleId}`);
      }
      
      // Perform detailed analysis
      return await this.analyzer.analyzeRoleCompatibility(cv, profile);
    } catch (error) {
      console.error('Role compatibility analysis failed:', error);
  ...
  private async performRoleMatch(cv: ParsedCV, profile: RoleProfile): Promise<RoleMatchResult> {
    ...
    // Use the matcher for final scoring - use the primary role confidence
    const matchScore = analysis.primaryRole.confidence;

    return {
      roleId: profile.id,
      roleName: profile.name,
      confidence: matchScore,
      matchingFactors: analysis.primaryRole.matchingFactors,
      enhancementPotential: analysis.primaryRole.enhancementPotential,
      recommendations: analysis.primaryRole.recommendations,
      scoringReasoning: analysis.primaryRole.scoringReasoning,
